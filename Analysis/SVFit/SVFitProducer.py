...
class SVFitProducer(Module):
    def __init__(self):
        ...
        for baseName in ["MeasuredTauLepton", "ClassicSVFit", "svFitHistogramAdapter", "svFitAuxFunctions"]: 
    ...
    def analyze(self, event):
        ...
        if event.ETau_HavePair:
            ...
            measTau2 = ROOT.MeasuredTauLepton(1, event.Tau_pt[idx], event.Tau_eta[idx], event.Tau_phi[idx], event.Tau_mass[idx], event.Tay_decayMode[idx])
            ...
            measTau1 = ROOT.MeasuredTauLepton(1, event.Tau_pt[idx], event.Tau_eta[idx], event.Tau_phi[idx], event.Tau_mass[idx], event.Tay_decayMode[idx])
            idx = event.TauTau_Tau1Idx
            measTau2 = ROOT.MeasuredTauLepton(1, event.Tau_pt[idx], event.Tau_eta[idx], event.Tau_phi[idx], event.Tau_mass[idx], event.Tay_decayMode[idx])
        
        
        if measTau1 not None and measTau2 not None:
